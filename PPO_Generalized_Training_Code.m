...
rng('default')
%% Create Agent PPO
% A PPO agent approximates the long-term reward given observations and actions using a critic value function representation. 
...
%% Actor Netwrok
% A PPO agent decides which action to take given observations by using an actor representation.
% To create the actor, first create a deep neural network with one input, the observation, and one output, the action.
% Construct the actor similarly to the critic. For more information, see rlContinuousGaussianActor.
...
%% Agent Options
% To create the PPO agent, first specify the PPO agent options using rlPPOAgentOptions.
agentOptions = rlPPOAgentOptions(...
    'SampleTime',T_Sample, ...
     ExperienceHorizon=1024,ClipFactor=0.04, EntropyLossWeight=0.1,NumEpoch=3
     AdvantageEstimateMethod="gae",GAEFactor=0.5, ...
     DiscountFactor=0.997,ActorOptimizerOptions=actorOptions,CriticOptimizerOptions=criticOptions);

% Then, create the PPO agent using the specified actor representation, critic representation, and agent options. 
% For more information, see rlPPOAgent.
